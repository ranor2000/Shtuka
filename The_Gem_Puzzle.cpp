...
void score(sp *head,int n, int v){
	...
	outtextxy(40,getmaxy()/2-160,"Enter youre");
	outtextxy(180,getmaxy()/2-80,"name:");
	bar(40,getmaxy()/2,90,getmaxy()/2+60);
	outtextxy(40,getmaxy()/2+80,"Score:");
	itoa(n,sc,10);
...
int main(int argc, char** argv) {
	...
	while(1){
		rec=new ba[16];
		n=0;
		outtextxy(100,getmaxy()/2-80,"Press<F>");
		outtextxy(100,getmaxy()/2,"To Start");
		a=getch();
		if(a==27)break;
		if(a==102){
			newboard(rec);
